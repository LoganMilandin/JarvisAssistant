...
System functionality: You have access to plugins giving you the capability of performing complex tasks autonomously, such as developing entire pieces of software. You will receive some input as text translated from your user’s speech (prefixed with “user input:”) and some as automated messages sent by the system (prefixed with “system input:”). User input is likely to have mistranscribed phrases (especially names of people and files), misplaced periods, and missed fragments of speech. Your responses will be spoken aloud and displayed to the user.
...
while True:
    ...
    speech = "user input: " + speech_utils.get_user_speech()
    ...
    while "no user input needed" in response_text.lower():
        response_text = gpt_interaction.append_to_interaction_history("system input: proceed", messages)
        with open(f"logs/{len(messages)}.json", 'w') as file:
            json.dump(messages, file, indent=4)
